...
for (x_l in c(40,50,60,70,80)){
  ...
  dat <- dat[rep(seq_len(n_row), each=81),]

  tij_seq_SBP <- rep(seq(x_l-10, x_l,length=81),n_row)
  ...
  dat<-data.frame(dat[,c("id","eventtime", "status", "tij_SBP", "Yij_SBP", "Xij_SBP", "tij_TCHDL", "Yij_TCHDL", "Xij_TCHDL","Age_start")],
                  Ethnicity,
                  Smoking,
                  Diabetes,
                  NZDep,
  ...
  dat_LME_LOCF<-dat[dat$tij_SBP %in% seq(x_l-10, x_l-10/obs,length=obs),]
  ...
  dat_LME_F_x_l<-fit_LME_landmark(data_long=dat_LME_LOCF,
                                  ...
                                  standardise_time_sd=sd_response_time_F,x_l=x_l,obs=obs,seed=seed,random_slope_survival = FALSE
  )
  ...
  saveRDS(dat_gold_F_x_l[[1]],file=paste0("/rds/project/rds-csoP2nj6Y6Y/ib401/dat_mis_gold_F/dat_mis_gold_F_",x_l,"/dat_mis_gold_LOCF_F_",x_l,"_",seed,".RDS"))
}
