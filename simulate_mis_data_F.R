...
tau_SBP<-(runif(n,min=55,max=65)-mean_response_time_F)/sd_response_time_F
tau_TCHDL<-tau_SBP+(10/sd_response_time_F)
...
for (m in long_names){
  ...
  betas[[name_response]][[paste0("beta_2_",m)]]<-get(paste0("beta_2_",m))
  betas[[name_response]][[paste0("tau_",m)]]<-get(paste0("tau_",m))
}
...
saveRDS(dat,file=paste0("/rds/user/ib401/hpc-work/dat_mis_simsurv/dat_mis_simsurv_",seed,".RDS"))
